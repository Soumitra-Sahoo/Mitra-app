    ...
    if (group_id) {
      ...
      if (check.group) {
        check.group.members
          ...
          .forEach((m) =>
            pushEvent(m.user_id, { type: "typing", from, isTyping, group_id }),
          );
      }
    } else {
  ...
  try {
    ...
    if (reply_to) {
      ...
      const belongsToThread = group_id
        ...
        : original &&
          ((original.from_user_id === userId &&
            original.to_user_id === to_user_id) ||
            (original.from_user_id === to_user_id &&
              original.to_user_id === userId));
...
export const forwardMessage = async (req, res) => {
  try {
    ...
    if (to_user_ids.length === 0 && to_group_ids.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Pick at least one recipient" });
    }
    ...
    let authorized =
      original.from_user_id === userId || original.to_user_id === userId;
    ...
    const forwardedType =
      original.message_type === "call" || original.message_type === "system"
        ? "text"
...
export const getUnreadMessageCount = async (req, res) => {
  try {
    const { userId } = req.auth();
    const [dmCount, groupMessages] = await Promise.all([
    ...
    res.json({ success: true, count: dmCount + groupMessages.length });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
