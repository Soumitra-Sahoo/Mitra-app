    ...
    <div className='fixed inset-0 z-[110] min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
      <div className='w-full max-w-md'>
        ...
        <div className='flex gap-2 mt-4'>
          <button onClick={() => { setMode('text'); setMedia(null); setPreviewUrl(null); }} className={`flex-1 flex font-semibold items-center justify-center gap-2 p-2 cursor-pointer rounded ${mode === 'text' ? "bg-white text-slate-900" : "bg-white/10"}`}>
            <TextIcon size={18} /> Text
          </button>
          <label className={`flex-1 flex items-center justify-center gap-2 p-2 rounded cursor-pointer ${mode === 'media' ? "bg-white text-slate-900" : "bg-white/10"}`}>
        ...
        <button
          ...
          disabled={saving}
          className='flex items-center justify-center gap-2 text-white py-3 mt-4 w-full rounded bg-gradient-to-r from-gradient-start to-gradient-end hover:brightness-110 active:scale-95 transition cursor-pointer disabled:opacity-60'
        >
