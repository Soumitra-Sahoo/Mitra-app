...
import {
  dummyFollowersData as connections,
    ...
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <h1 className='text-3xl font-bold text-slate-900 mb-2'>Connections</h1>
          <p className='text-slate-600'>Explore connections..</p>
            </div>
                ...
                rounded-md p-1 bg-white shadow-sm'>
                  {
                    dataArray.map((tab)=>(
                      <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`cursor-pointer flex items-center px-3 py-1 text-sm 
                        rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 
