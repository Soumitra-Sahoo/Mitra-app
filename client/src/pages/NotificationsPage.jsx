...
const typeIcon = (type) => {
  switch (type) {
    case "like":
      return <Heart className="size-4 text-rose fill-rose" />;
    case "comment":
    case "reply":
    ...
    case "connection_request":
      return <Users className="size-4 text-secondary-accent" />;
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground flex items-center gap-2">
              ...
              {unread > 0 && (
                <span className="text-sm font-semibold bg-danger text-white px-2 py-0.5 rounded-full">
                  {unread}
                </span>
              )}
            </h1>
          ...
          {unread > 0 && (
            <button
              onClick={markAllRead}
              className="flex items-center gap-2 text-sm text-primary hover:text-primary-active font-medium transition"
            >
          ...
          {notifications.map((notif) => (
            <div
              ...
              className={`flex items-start gap-3 p-4 rounded-2xl cursor-pointer transition-all hover:shadow-md
                ${notif.read ? "bg-card" : "bg-primary/10 border border-primary/30"}`}
            >
              {/* Avatar */}
              <div className="relative flex-shrink-0">
                <img
                  ...
                  alt=""
                />
                <span className="absolute -bottom-1 -right-1 bg-card rounded-full p-0.5 shadow">
                  {typeIcon(notif.type)}
                </span>
              </div>
              ...
              {/* Unread dot */}
              {!notif.read && (
                <span className="size-2 bg-primary rounded-full flex-shrink-0 mt-2" />
              )}
            </div>
          ))}
        </div>
